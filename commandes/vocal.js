...
module.exports = {
    ...
    run: async (client, message, args) => {
        ...
        if (!nom) return message.channel.send("Merci d'indiquer le nom du futur salon vocal.")

        message.guild.channels.create(nom, { type: "voice"})

        const embed = new Discord.MessageEmbed()
        .setTitle("<:luffy:866504382660476978> Salon vocal créé avec succès")
        .setDescription(`Le salon vocal a été crée avec succès\n\n\nVérifiez en haut de vos salons si vous voulez le déplacer dans une catégorie précise.`)
        .setColor("#07d4e5")
